...
class RemoteFolderService {
  ...
  Future<void> _handleListReq(String peerId, Map<String, dynamic> data) async {
    ...
    final collaborator =
        await (_db.select(_db.spaceCollaborators)..where(
              (t) =>
  ...
  StreamController<List<RemoteFile>> _getFileController(String spaceId) {
    if (!_remoteFileControllers.containsKey(spaceId)) {
      _remoteFileControllers[spaceId] =
          StreamController<List<RemoteFile>>.broadcast();
    }
  ...
  Future<void> _updateSpaceMetadata(
    String spaceId,
    String name,
    String permission,
  ) async {
    await (_db.update(
      _db.sharedFolders,
    )..where((t) => t.id.equals(spaceId))).write(
      SharedFoldersCompanion(name: Value(name), permission: Value(permission)),
    );
  }
  ...
  Future<void> _handleDownloadAck(
    String peerId,
    Map<String, dynamic> data,
  ) async {
    final spaceId = data['spaceId'];
    ...
    final downloadPath = await StorageService.getStoaDownloadsPath(
      'SharedSpaces/${folder.name}',
    );
  ...
  Future<void> _handleFetchContentReq(
    String peerId,
    Map<String, dynamic> data,
  ) async {
    final spaceId = data['spaceId'];
  ...
  void _handleChangeNotify(String peerId, Map<String, dynamic> data) {
    final spaceId = data['spaceId'];
    debugPrint(
      '🔔 Change notification for space $spaceId: ${data['action']} ${data['path']}',
    );
  ...
  Future<void> inviteCollaborator(
    ...
    final folder = await (_db.select(
      _db.sharedFolders,
    )..where((t) => t.id.equals(spaceId))).getSingle();
  ...
  Future<List<Map<String, dynamic>>> _listFilesRecursive(String dirPath) async {
    ...
    await for (final entity in dir.list(recursive: true)) {
      ...
      if (entity is File) {
        ...
        result.add({'path': relativePath, 'size': stat.size, 'isDir': false});
      } else if (entity is Directory) {
        result.add({'path': relativePath, 'size': 0, 'isDir': true});
      }
    }
