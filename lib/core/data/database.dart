...
class Groups extends Table {
  ...
  BoolColumn get showHistoryToNew =>
...
class SharedFolders extends Table {
  ...
  TextColumn get permission => text().withDefault(
    const Constant('read-write'),
...
class AppDatabase extends _$AppDatabase {
  ...
  Future<void> removeGroupMember(String groupId, String peerId) {
    return (delete(
      groupMembers,
    )..where((t) => t.groupId.equals(groupId) & t.peerId.equals(peerId))).go();
  }
  ...
  Future<bool> isPeerInAnyGroup(String peerId) async {
    final member =
        await (select(groupMembers)..where(
              (t) => t.peerId.equals(peerId) & t.status.equals('accepted'),
            ))
            .getSingleOrNull();
    return member != null;
  }
  ...
  Future<int> clearMessagesForGroup(String groupId) {
    return (delete(
      groupMessages,
  ...
  Future<void> updateGroupMessageFile(
    String groupId,
    String senderId,
    String filename,
    String path,
    ...
    appLogger.i(
      '🔍 Attempting to update group message: groupId=$groupId, sender=$senderId, file=$filename',
    );
    ...
    for (int i = 0; i < 3; i++) {
      ...
      final query = select(groupMessages)
        ..where(
          (t) =>
              ...
              t.content.equals(filename),
        ) // Removed loose type check
        ..orderBy([
      ...
      if (message != null) {
        ...
        await (update(
          ...
          GroupMessagesCompanion(
            filePath: Value(path),
            type: Value(
              path.endsWith('.zip') || Directory(path).existsSync()
                  ? 'folder'
                  : 'file',
      ...
      appLogger.i(
        '⚠️ Message not found (attempt ${i + 1}/3), retrying in 500ms...',
      );
      await Future.delayed(const Duration(milliseconds: 500));
    }

    appLogger.i(
      '❌ Failed to find matching group message for file update after retries.',
    );
  }
  ...
  Stream<List<SharedFolder>> watchAllSharedFolders() {
    return (select(sharedFolders)..orderBy([
  ...
  Future<void> purgeIgnoredFiles() async {
    final rowsDeleted =
        await (delete(sharedFiles)..where((t) {
              return t.relativePath.like('.%') |
                  t.relativePath.like('%/.%') |
                  t.relativePath.like('%~') |
                  t.relativePath.like('%.tmp') |
                  t.relativePath.like('%.swp') |
                  t.relativePath.like('%Thumbs.db');
            }))
            .go();

    if (rowsDeleted > 0) {
      appLogger.i('🧹 Purged $rowsDeleted ignored files from database.');
    }
  }
  ...
  Future<int> addCollaborator(SpaceCollaboratorsCompanion collaborator) {
    return into(
      spaceCollaborators,
    ).insert(collaborator, mode: InsertMode.insertOrReplace);
  }

  Future<void> removeCollaborator(String spaceId, String peerId) {
    return (delete(
      spaceCollaborators,
    )..where((t) => t.spaceId.equals(spaceId) & t.peerId.equals(peerId))).go();
  }

  Stream<List<SpaceCollaborator>> watchCollaborators(String spaceId) {
    return (select(spaceCollaborators)..where(
          (t) => t.spaceId.equals(spaceId) & t.status.equals('accepted'),
        ))
        .watch();
  }
  ...
  Future<void> updateCollaboratorStatus(
    String spaceId,
    String peerId,
    String status,
  ) {
  ...
  Stream<List<SharedFolder>> watchSpacesAsCollaborator(String myPeerId) {
    ...
    final collaboratorSpaceIds = selectOnly(spaceCollaborators)
      ...
      ..where(
        spaceCollaborators.peerId.equals(myPeerId) &
            spaceCollaborators.status.equals('accepted'),
      );

    return (select(
      sharedFolders,
    )..where((t) => t.id.isInQuery(collaboratorSpaceIds))).watch();
  }
}
