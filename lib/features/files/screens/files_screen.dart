...
class _FilesScreenState extends ConsumerState<FilesScreen> with SingleTickerProviderStateMixin {
        ...
        leading: _currentGroupId != null || _downloadsPath.isNotEmpty
          ? IconButton(
              ...
              onPressed: () {
                ...
                } else if (_tabController.index == 1 && _downloadsPath.isNotEmpty) {
  ...
  String _getTitle(AppDatabase db) {
    if (_tabController.index == 0) {
      return _currentGroupId == null ? 'Files' : _getGroupName(db, _currentGroupId!);
    } else {
              ...
              return Center(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      _downloadsPath.isEmpty ? 'No downloads yet' : 'Empty folder',
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    child: ListTile(
                      ...
                      title: Text(name),
                      subtitle: Text(isDir ? 'Folder' : _formatBytes(stat.size)),
                      trailing: isDir 
                        ? const Icon(Icons.chevron_right)
                        : IconButton(
                            icon: const Icon(Icons.delete, color: Colors.red),
                            onPressed: () => _confirmDeleteDownload(entity.path),
                          ),
                      onTap: () {
                        if (isDir) {
